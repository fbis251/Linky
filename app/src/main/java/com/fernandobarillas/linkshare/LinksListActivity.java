...
public class LinksListActivity extends AppCompatActivity {
    ...
    private void getList() {
        ...
        call.enqueue(new Callback<LinksList>() {
            ...
            public void onResponse(Call<LinksList> call, Response<LinksList> response) {
                ...
                if (!response.isSuccessful()) {
                    String message = "Invalid response returned by server: " + ResponsePrinter.httpCodeString(response);
                ...
                adapterSetup();
                mSwipeRefreshLayout.setRefreshing(false);
                String message = String.format("Downloaded %d %s", LinkStorage.getLinksCount(), LinkStorage.getLinksCount() == 1 ? "link" : "links");
    ...
    private void touchHelperSetup() {
        ...
        ItemTouchHelperCallback callback = new ItemTouchHelperCallback(new ItemSwipedRightCallback() {
            @Override
